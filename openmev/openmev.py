...
class OpenmevRPC:
    ...
    eth_sendPrivateTransaction = RPCEndpoint("eth_sendRawTransaction")
    eth_cancelPrivateTransaction = RPCEndpoint("eth_cancelRawTransaction")


class OpenmevBundleResponse:
    w3: Web3
...
class OpenmevPrivateTransactionResponse:
    w3: Web3
    tx: SignedTxAndHash
...
class Openmev(Module):
    signed_txs: List[HexBytes]
    response: Union[OpenmevBundleResponse, OpenmevPrivateTransactionResponse]

    def sign_bundle(
        self,
        bundled_transactions: List[
            Union[OpenmevBundleTx, OpenmevBundleRawTx, OpenmevBundleDictTx]
        ],
    ) -> List[HexBytes]:
        ...
        for tx in bundled_transactions:
            print(tx)
            if "signed_transaction" in tx:  # OpenmevBundleRawTx
            ...
            elif "signer" in tx:  # OpenmevBundleTx
            ...
            elif all(key in tx for key in ["v", "r", "s"]):  # OpenmevBundleDictTx
                v, r, s = (
                    tx["v"],
    ...
    def send_raw_bundle_munger(
        ...
        target_block_number: int,
        opts: Optional[OpenmevOpts] = None,
    ) -> List[Any]:
    ...
    sendRawBundle: Method[Callable[[Any], Any]] = Method(
        OpenmevRPC.eth_sendBundle, mungers=[send_raw_bundle_munger]
    )
    send_raw_bundle = sendRawBundle

    def send_bundle_munger(
        self,
        bundled_transactions: List[Union[OpenmevBundleTx, OpenmevBundleRawTx]],
        target_block_number: int,
        opts: Optional[OpenmevOpts] = None,
    ) -> List[Any]:
        signed_txs = self.sign_bundle(bundled_transactions)
        self.response = OpenmevBundleResponse(
    ...
    def extrapolate_timestamp(self, block_tag: int, latest_block_number: int):
        ...
        # print(dict(self.web3.eth.get_block(latest_block_number)))
        return dict(self.web3.eth.get_block(latest_block_number))["timestamp"] + (
    ...
    def call_bundle_munger(
        ...
        signed_bundled_transactions: List[
            Union[OpenmevBundleTx, OpenmevBundleRawTx]
        ],
        evm_block_number,
        evm_block_state_number,
        evm_timestamp,
    ...
    call_bundle: Method[Callable[[Any], Any]] = Method(
        json_rpc_method=OpenmevRPC.eth_callBundle, mungers=[call_bundle_munger]
    )
    ...
    def send_private_transaction_munger(
        self,
        transaction: Union[OpenmevBundleTx, OpenmevBundleRawTx],
        max_block_number: Optional[int] = None,
    ) -> Any:
        """Sends a single transaction to Openmev.
        If `max_block_number` is set, Openmev will try to submit the transaction in every block <= that block (max 25 blocks from present)."""
        ...
        params = self.to_hex(signed_transaction)
        # print(params)
        self.response = OpenmevPrivateTransactionResponse(
            self.web3, signed_transaction, max_block_number
        )
        return [params]

    sendPrivateTransaction: Method[Callable[[Any], Any]] = Method(
        json_rpc_method=OpenmevRPC.eth_sendPrivateTransaction,
    ...
    def cancel_private_transaction_munger(
        ...
        """Stops a private transaction from being sent to miners by Openmev.
    ...
    cancelPrivateTransaction: Method[Callable[[Any], Any]] = Method(
        json_rpc_method=OpenmevRPC.eth_cancelPrivateTransaction,
